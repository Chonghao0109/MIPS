//Project Name  :MIPS 5-Pipeline Design
//File Name     :core.v
//Designer      :ChongHao Xu
//Email         :hsuhowardhsu@icloud.com
//Description   :
//
//
//
...
module Multi_Cycle_Processer(clk,rst);
    input clk,rst;
    ...
    IF IF(.clk(clk),.rst(rst),.PCstc(MEM_PCstc),.zero(MEM_zero),.PC(IF_PC),.t0(IF_PCa4),.t1(MEM_t1),.t2(MEM_t2),.instruction(IF_instruction));
	IFIDReg IFIDReg(.clk(clk),.rst(rst),.PC(IF_PCa4),.instruction(IF_instruction),.nPC(ID_PC),.ninstruction(ID_instruction));
	//----------------------------------------------------------
	...
	//------------------instruction decode----------------------
	controlunit controlunit(.ins(ID_instruction[31:26]),.funct(ID_instruction[5:0]),
    ...
    gpr gpr(.clk(clk),.RegWrite(WB_RegWrite),.rst(rst),.RW(WB_rw),.RA(ID_instruction[25:21]),.RB(ID_instruction[20:16]),.busW(WB_busW),.busA(ID_A),.busB(ID_B));

	IDEXEReg IDEXEReg(	.clk(clk),.rst(hazardclr),.MemtoReg(ID_MemtoReg),
	...
	MEMWBReg MEMWBReg(.clk(clk),.rst(rst),.MemtoReg(MEM_MemtoReg),.RegWrite(MEM_RegWrite),.result(MEM_result),.dataout(MEM_dataout),.rw(MEM_rw),
...
module IF(clk,rst,PCstc,zero,PC,t2,t1,t0,instruction);
	...
	always@(*)begin
		if(stall) instruction=0;
		else instruction={instruction_memory[PC],instruction_memory[PC+1],instruction_memory[PC+2],instruction_memory[PC+3]};
	end
	...
	always@(posedge clk or posedge rst)begin
		...
		else if(instruction[31:26]==6'b100011) begin stall=1; end
		else begin 
			PC = newPC; 
			stall=0;
		end
...
module ALU (A,B,ALUop,zero,result,carryout);
	...
	always@(*)begin
		case(ALUop[1:0])
			2'b00:result = na | nb ;
			2'b01:result = na & nb ;
			2'b10:{carryout,result} = na + nb + ALUop[2]|ALUop[3];
...
module Memory(clk,MemWrite,MemRead,rst,Adder,datain,dataout);
	...
	always@(posedge clk or posedge rst)begin
		...
		else if(MemWrite) {Mem[pointer],Mem[pointer+1],Mem[pointer+2],Mem[pointer+3]}<=datain;
...
module maincontroler(opcode,RegDst,ALUsrc,MemtoReg,RegWrite,MemWrite,Memread,PCstc,preopcode);  
	...
	always@(opcode)begin
		case(opcode)
			//R-format
			6'b00_0000:	{RegDst,ALUsrc,MemtoReg,RegWrite,MemWrite,Memread,PCstc,preopcode}=11'b100100_00_111;//R-type

			//i-format
			6'b00_1000:	{RegDst,ALUsrc,MemtoReg,RegWrite,MemWrite,Memread,PCstc,preopcode}=11'b010100_00_000;//addi add
			6'b00_1100:	{RegDst,ALUsrc,MemtoReg,RegWrite,MemWrite,Memread,PCstc,preopcode}=11'b010100_00_010;//andi and
			6'b00_1101:	{RegDst,ALUsrc,MemtoReg,RegWrite,MemWrite,Memread,PCstc,preopcode}=11'b010100_00_011;//ori or

			6'b10_0011:	{RegDst,ALUsrc,MemtoReg,RegWrite,MemWrite,Memread,PCstc,preopcode}=11'b011101_00_000;//lw  add
			6'b10_1011:	{ALUsrc,RegWrite,MemWrite,Memread,PCstc,preopcode}=10'b1010_00_000;//sw  add

			6'b00_0100:	{ALUsrc,RegWrite,MemWrite,Memread,PCstc,preopcode}=9'b0000_01_001;//beq sub
			
			//j-format
			6'b00_0010: PCstc=2'b10;
...
module ALUcontroler(preopcode,funct,ALUop); 
	...
	always@(preopcode or funct)begin
		casex({preopcode,funct})

			9'b000_xxxxxx: ALUop=4'b0010; //add
			9'b001_xxxxxx: ALUop=4'b0110; //sub
			9'b010_xxxxxx: ALUop=4'b0001; //and
			9'b011_xxxxxx: ALUop=4'b0000; //or

			9'b111_100000: ALUop=4'b0010; //add
			9'b111_100010: ALUop=4'b0110; //sub
			9'b111_100100: ALUop=4'b0001; //and
			9'b111_100101: ALUop=4'b0000; //or
			9'b111_101010: ALUop=4'b0111; //set on less than
		endcase
	end
...
module IFIDReg(
	...
	always@(posedge clk or posedge rst)begin
		if(rst) {nPC,ninstruction} = 0;
		else {nPC,ninstruction} = {PC,instruction};
	end
...
module forwards(As,Bs,A_addr,B_addr,MEM_addr,WB_addr,MEM_WB,WB_WB);
	...
	always@(*)begin
		if((A_addr==MEM_addr)&&(MEM_WB)) As=1;
		else if((A_addr==WB_addr)&&(WB_WB)) As=2;
		else As=0;

		if((B_addr==MEM_addr)&&(MEM_WB)) Bs=1;
		else if((B_addr==WB_addr)&&(WB_WB)) Bs=2;
		else Bs=0;
	end
